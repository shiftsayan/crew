...
function Setup(ctx, setupData) {
  ...
  for (let num = 1; num <= 4; num++) cards.push({ num: num, suite: "black" });
...
function DealCards(G, ctx) {
  ...
  while (G.cards.length !== 0) {
    ...
    if (card.suite === "black" && card.num === 4) G.commander = player;
...
function ChooseGoals(G, ctx, idx) {
  if (G.goals[idx].player !== null) throw new Error("Goal Already Chosen");
...
function Communicate(G, ctx, isCommunicating, card, order) {
  ...
  if (card.suite === "black") throw new Error("Cannot Communicate Rockets"); // TODO

  G.player[ctx.currentPlayer].canCommunicate = false;
  G.player.communication = {
    card: card,
    order: G.reception_dead_spot ? null : order,
  };
}
