    ...
    return (
        <div className={classnames({
            ...
            <div className="relative">
                <CrewPill num={goal.num} suite={goal.suite} blank={decoration === Decoration.Pending} decorations={decorations} />
                {goal.order !== Order.None &&
                    <CrewPendant icon={mapOrderToIcon[goal.order]} suite={goal.suite} decorations={decorations} />
                }
            </div>
        </div>
    )
...
function CrewPendant({ icon, suite, decorations }) {
    return (
        <div className={classnames({
            "w-5 h-5 bg-white absolute inset-x-1/2 inset-y-full -mx-2.5 -my-2.5 rounded-full flex justify-center border-2": true,
