...
export function toggleGoal(goal_idx, state) {
  ...
  if (
    state.goals[goal_idx].player !== undefined &&
    state.phase !== OldPhase.GoldenBorderDiscard
  ) {
  ...
  var player_goals = [];
  if (selection) player_goals = state.players[new_player].goals.concat(goal);
  else
    player_goals = state.players[previous_player].goals.filter(
