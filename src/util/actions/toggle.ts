...
export class Toggle extends Move {
    async validateParams(goal_idx) {
        ...
        return this.game.goals[goal_idx].player === undefined || this.game.phase === PhaseName.GoldenBorderDiscard
    }
    ...
    updateGame(goal_idx) {
        ...
        if (selection) {
            ...
            const player_goals = (this.game.players[this.state.player].goals ?? []).concat(goal)
            ...
            return {
                ...
                golden_border: this.game.golden_border === GoldenBorder.Using ? GoldenBorder.Used : this.game.golden_border,
            }
        }
        else {
            const goals = [...this.game.goals]
            ...
            const player_goals = (this.game.players[previous_player].goals ?? []).filter((_goal) => !(_goal.num === goal.num && _goal.suite === goal.suite))
